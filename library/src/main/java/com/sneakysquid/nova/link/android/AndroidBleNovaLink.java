 ...
 */
public class AndroidBleNovaLink implements NovaLink {
    ...
    protected void onScannedDevice(BluetoothDevice device, int rssi, byte[] scanRecord) {
        ...
        if (isNova(device)) {
            ...
            this.connected = device.connectGatt(activity, false /* first connect false, subsequent true */,
                    ...
                    new BluetoothGattCallback() {
                        ...
                        public void onConnectionStateChange(final BluetoothGatt gatt, final int status, final int newState) {
                            activity.runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    AndroidBleNovaLink.this.onConnectionStateChange(gatt, status, newState);
                                }
                            });
                        }
                        ...
                        public void onServicesDiscovered(final BluetoothGatt gatt, final int status) {
                            activity.runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    AndroidBleNovaLink.this.onServicesDiscovered(gatt, status);
                                }
                            });
                        }
                        ...
                        public void onCharacteristicWrite(final BluetoothGatt gatt, final BluetoothGattCharacteristic characteristic, final int status) {
                            activity.runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    AndroidBleNovaLink.this.onCharacteristicWrite(gatt, characteristic, status);
                                }
                            });
                        }
                    });
    ...
    public void flash(NovaFlashCommand flashCmd, NovaFlashCallback callback) {
        ...
        debug("written characteristic: %s", characteristic);
